<!DOCTYPE html>
<html>
<head>
	<title>Surat Perjanjian Tugas</title>
	<style type="text/css">
  body{
    font-family: "Times New Roman", Times, serif;
    max-width: 100%;
  }
  p{
    font-size: 14px;
  }
	.text-center{
    text-align:center;
  }
  h4,h5{
    margin-bottom:5px;
    margin-top:5px;
  }
  #header hr{
    background-color:#000;
  }

  #perihal{
    margin:auto;
    margin-top:35px;
    width:70%;
  }
  #perihal .nomor{
    margin-left:140px;
  }
  #perihal p{
    margin-top:3px;
  }
  #isi{
    margin-top:35px;
    width:100%;
  }
  #isi .paragraf{
    text-indent: 50px;
  }
  
  .clearfix::after {
    content: "";
    clear: both;
    display: table;
  }

  table {
    border-collapse: collapse;
    border : none;
  }
  table thead tr th,
  table tbody tr td {
    font-size: 12px;
    padding: 0px;
  }
  .text-justify{
    text-align: justify;
  }
	</style>
</head>
<body>
  <?php 
    //Array Hari
    $array_hari = array(1=>"Senin","Selasa","Rabu","Kamis","Jumat", "Sabtu","Minggu");
    $hari = $array_hari[date('N')];

    //Format Tanggal
    $tanggal = date ('j');

    //Array Bulan
    $array_bulan = array(1=>"Januari","Februari","Maret", "April", "Mei", "Juni","Juli","Agustus","September","Oktober", "November","Desember");
    $bulan = $array_bulan[date('n')];

    //Format Tahun
    $tahun = date('Y');

    $date   = explode('-', $data_list->tgl_awal_sewa);
    $thn    = $date[0];
    $bln  = $date[1];
    $hari   = $date[2];
    $tgl_awal = $hari.' - '.$bln.' - '.$thn;

    $date1   = explode('-', $data_list->tgl_akhir_sewa);
    $thn1    = $date1[0];
    $bln1  = $date1[1];
    $hari1   = $date1[2];
    $tgl_akhir = $hari1.' - '.$bln1.' - '.$thn1;

    $image_tegal = "data:image/png;base64,";
  ?>
  <div id="header">
    <table style="width:100%;">
      <tr>
        <td valign="top" style="width:20%;text-align:right;">
          <img alt="Logo Pemkot Tegal" style="width:100px;height:100px;" src="<?php echo $image_tegal;?>">
        </td>
        <td valign="top" style="width:80%;">
          <h3 class="text-center">PEMERINTAH KOTA TEGAL</h3>
          <h3 class="text-center">DINAS PEKERJAAN UMUM DAN PENATAAN RUANG</h3>
          <h4 class="text-center">Jl. Proklamasi Nomor 11 Tegal</h4>
          <h4 class="text-center">Telp./Fax. (0283) 356353 Tegal - 52111</h4>
        </td>
      </tr>
    </table>
    
    <hr size="4" />
  </div>

  <div id="perihal">
    <p class="text-center">SURAT PERJANJIAN SEWA MENYEWA MESIN <?php echo strtoupper($data_list->nm_kendaraan);?></p>
    <p class="text-center">MILIK  PEMERINTAH  KOTA  TEGAL</p>
    <hr width="80%" style="margin-top:15px;" />
    <p class="nomor">Nomor:</p>
  </div>

  <div id="isi">
    <p class="paragraf text-justify">Pada hari ini <?php echo $hari.', ',$tanggal.' '.$bulan.' '.$tahun;?>, yang bertanda tangan di bawah ini :</p>
    <table style="width:100%">
      <tr>
        <td valign="top" style="width:30%">
          <p>1. Nama : SUGIYANTO,ST.MT </p>
        </td>
        <td valign="top" style="width:5%;text-align:right;">
          <p>:</p>
        </td>
        <td valign="top" style="width:60%">
          <p class="text-justify">Kepala Dinas Pekerjaan Umum Dan Penataan Ruang Kota Tegal – bertindak atas nama Walikota Tegal yang selanjutnya di sebut : <u>PIHAK PERTAMA</u>.</p>
        </td>
      </tr>
      <tr>
        <td valign="top" style="width:30%">
          <p>1. Nama : <?php echo $data_list->nm_penyewa;?> </p>
        </td>
        <td valign="top" style="width:5%;text-align:right;">
          <p>:</p>
        </td>
        <td valign="top" style="width:60%">
          <p class="text-justify">Bertempat Tinggal di <?php echo $data_list->alamat_ktp;?><br />
        Bertindak untuk atas nama Penyewa <br />
        Selanjutnya disebut <u>PIHAK KEDUA</u>.</p>
        </td>
      </tr>

    </table>
  </div>

  <div id="peraturan">
      <p class="text-justify">Berdasarkan Peraturan Daerah Kota Tegal Nomor : 2 Tahun 2012 tanggal 11 Januari 2012 tentang Retribusi Jasa Usaha, kedua belah pihak telah sepakat mengadakan perikatan/perjanjian sebagai berikut :</p>
      <p class="text-center"><strong>Pasal 01<strong></strong></strong>
      <p class="text-justify">PIHAK PERTAMA menyewakan kepada PIHAK KEDUA sebuah Mesin <?php echo $data_list->nm_kendaraan.' '.$data_list->jns_kendaraan;?>.</p> 

      <p class="text-center"><strong>Pasal 02<strong></strong></strong>
      <p class="text-justify">Mesin Gilas tersebut di pakai untuk keperluan sesuai dengan fungsinya dan tidak boleh di pergunakan untuk keperluan lain.</p> 

      <p class="text-center"><strong>Pasal 03<strong></strong></strong>
      <p class="text-justify">Jangka waktu pemakaian Mesin <?php echo $data_list->nm_kendaraan;?> tersebut pada pasal 01 selama <?php echo $data_list->renc_pemakaian;?>, terhitung mulai tanggal <?php echo $tgl_awal;?>.sampai dengan tanggal <?php echo $tgl_akhir;?>.</p>

      <p class="text-center"><strong>Pasal 04<strong></strong></strong>
      <p class="text-justify">Biaya sewa dihitung mulai  hari  berangkat/ keluar dari kantor menuju ke lokasi termasuk Hari Libur  dan Hari Minggu ( hari kalender ).</p> 

      <p class="text-center"><strong>Pasal 05<strong></strong></strong>
      <p class="text-justify">Biaya sewa seluruhnya sebesar Rp <?php echo $data_list->biaya_sewa;?>,- x <?php echo $data_list->lama;?> hari = Rp <?php echo $data_list->biaya_sewa * $data_list->lama;?>,-</p>

      <p class="text-center"><strong>Pasal 06<strong></strong></strong>
      <p class="text-justify">Uang sewa harus di bayar di muka selama : .......................... hari kepada PIHAK PERTAMA dan di setorkan ke Kas Daerah Kota Tegal di Bank Pembangunan Daerah Jawa Tengah Cabang Tegal.</p>

      <p class="text-center"><strong>Pasal 07<strong></strong></strong>
      <p class="text-justify">Pelunasan pembayaran Retribusi sewa Mesin Gilas oleh PIHAK KEDUA sebagaimana tersebut dalam Pasal 06 (enam) Surat Perjanjian Sewa Menyewa Mesin Gilas ini , di bayar paling lambat tanggal ..................................</p>

      <p class="text-center"><strong>Pasal 08<strong></strong></strong>
      <p class="text-justify">Apabila pelunasan pembayaran Retribusi sewa Mesin Gilas sebagaimana tersebut dalam Pasal 07 (tujuh) diatas tidak di bayar tepat pada waktunya , maka PIHAK KEDUA di kenakan denda sebesar 1/1<sub>000</sub> (satu permil) untuk setiap harinya sampai dengan setinggi-tingginya 10% dari nilai sewa yang belum dibayar.</p>

      <p class="text-center"><strong>Pasal 08<strong></strong></strong>
      <p class="text-justify">Apabila pelunasan pembayaran Retribusi sewa Mesin Gilas sebagaimana tersebut dalam Pasal 07 (tujuh) diatas tidak di bayar tepat pada waktunya , maka PIHAK KEDUA di kenakan denda sebesar 1/1<sub>000</sub> (satu permil) untuk setiap harinya sampai dengan setinggi-tingginya 10% dari nilai sewa yang belum dibayar.</p>

      <p class="text-center"><strong>Pasal 09<strong></strong></strong>
      <p class="text-justify">Apabila dalam penggunaan alat sebagaimana tersebut pada Pasal 01 terjadi kerusakan dan atau hilang, maka pihak kedua wajib memperbaiki kerusakan tersebut atas biaya dari pihak kedua dan atau mengganti alat mesin/ yang hilang.</p>

      <p class="text-center"><strong>Pasal 10<strong></strong></strong>
      <p class="text-justify">Apabila dalam penggunaan alat sebagaimana tersebut pada Pasal 01 terjadi kerusakan dan atau hilang, maka pihak kedua wajib memperbaiki kerusakan tersebut atas biaya dari pihak kedua dan atau mengganti alat mesin/ yang hilang.</p>

      <p class="text-center"><strong>Pasal 11<strong></strong></strong>
      <p class="text-justify">PIHAK KEDUA tidak diperbolehkan melimpahkan hak sewa ini kepada pihak lain kecuali dengan ijin dan persetujuan kepada PIHAK PERTAMA.</p>

      <p class="text-center"><strong>Pasal 12<strong></strong></strong>
      <p class="text-justify">Apabila PIHAK KEDUA melanggar perjanjian sewa menyewa mesin Mesin Gilas ini maka PIHAK PERTAMA berhak untuk membatalkan surat perjanjian sewa menyewa ini secara sepihak dan PIHAK KEDUA di haruskan menyerahkan kembali Mesin <?php echo $data_list->nm_kendaraan;?> tersebut kepada PIHAK PERTAMA walaupun masa sewa belum berakhir.</p>

      <p class="text-center"><strong>Pasal 13<strong></strong></strong>
      <p class="text-justify">a.  Apabila di kemudian hari timbul sengketa antara PIHAK PERTAMA dan PIHAK KEDUA karena hal lain yang belum terdapat dalam surat perjanjian ini, maka di selesaikan secara musyawarah dan mufakat.<br/>
      b.   Apabila dengan cara demikian tidak mendapatkan hasil, maka atas dasar kepentingan umum/ Pemerintah Kota Tegal PIHAK PERTAMA dapat minta bantuan alat negara untuk mengambil tindakan pelaksanaan untuk atas surat keputusan PIHAK PERTAMA sebagai tindakan tersebut pada pasal 12 surat perjanjian ini.</p>

  </div>
  <div class="penutup">
    <p class="paragraf text-justify">Demikian surat perjanjian bermeterai ini di buat dan di tanda tangani oleh kedua belah pihak</p>
    <table style="width:100%;">
      <tr>
        <td valign="top" style="width:50%;">
          <p class="text-center" style="margin-bottom:135px;">PIHAK KEDUA</p>
          <p class="text-center"><?php echo $data_list->nm_penyewa;?></p>
        </td>
        <td valign="top"style="width:50%;">
          <p class="text-center">PIHAK PERTAMA</p>
          <p class="text-center" style="margin-bottom:100px;">Kepala Dinas PUPR Kota Tegal</p>
          <p class="text-center"><u>SUGIYANTO, ST.MT</u><br />
            Pembina Tk. I<br />
            NIP. 19621224 199303 1 007
          </p>
        </td>
      </tr>
    </table>
  </div>
  
</body>
</html>